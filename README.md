...
- [测试](#测试)
  ...
  - [按模块测试](#按模块测试)
  - [集成测试](#集成测试)
  - [性能压测（单元测试方式）](#性能压测单元测试方式)
  - [故障演练](#故障演练)
  - [Rust 引擎测试与基准测试](#rust-引擎测试与基准测试)
- [HTTP 接口压测](#http-接口压测)
  - [Step 1: 启动服务（压测模式）](#step-1-启动服务压测模式)
  - [Step 2: 冒烟测试](#step-2-冒烟测试)
  - [Step 3: wrk 压测](#step-3-wrk-压测)
  - [Step 4: 单接口 curl 压测](#step-4-单接口-curl-压测)
- [API 文档](#api-文档)
- [核心配置项](#核心配置项)
...
除了上述单元测试级别的压测，还可以对运行中的服务进行 HTTP 接口压测，更贴近真实生产场景。

### Step 1: 启动服务（压测模式）

压测模式会放开限流上限（100 万 QPS）、降低日志级别，避免限流和日志 I/O 成为瓶颈。

**IDEA 启动**：

1. Edit Configurations → 选择 `GatewayApplication`
2. 在 **VM options** 中添加：`-Dspring.profiles.active=bench`
3. 点击 Run
...
```bash
cd apexmatch-java
java -jar apexmatch-gateway/target/apexmatch-gateway-1.0.0-SNAPSHOT.jar --spring.profiles.active=bench
```
...
[1/5] 账户操作
  ✓ 充值 100000 USDT (HTTP 200)
  ✓ 查询账户余额 (HTTP 200)

[2/5] 下限价买单（挂单）
  ✓ 下限价买单 BTC@50000 (HTTP 200)

[3/5] 下限价卖单（触发撮合成交）
  ✓ 下限价卖单 BTC@50000 (部分成交) (HTTP 200)
...
运行一键压测脚本：
```bash
# 默认参数：10 秒 / 4 线程 / 100 并发
bash scripts/bench.sh

# 自定义参数：30 秒 / 8 线程 / 500 并发
bash scripts/bench.sh http://localhost:8080 30s 8 500
```

脚本会自动：
1. 为测试账户充值
2. 预挂 1000 笔限价单（确保有对手盘可撮合）
3. 压测下单接口 → 输出 TPS + 延迟分布（P50/P90/P95/P99/P999）
4. 压测盘口查询接口
5. 压测账户查询接口

### Step 4: 单接口 curl 压测

如果没有 wrk，也可以用 `ab`（Apache Bench，macOS 自带）：

```bash
# 准备请求体
echo '{"userId":1,"symbol":"BTC-USDT","side":"BUY","type":"LIMIT","price":50000,"quantity":0.1}' > /tmp/order.json

# 充值
curl -s -X POST "http://localhost:8080/api/v1/account/1/deposit?currency=USDT&amount=10000000"

# 压测下单：10000 次请求，100 并发
ab -n 10000 -c 100 -T 'application/json' -p /tmp/order.json \
   http://localhost:8080/api/v1/order/place

# 压测盘口查询：10000 次请求，200 并发
ab -n 10000 -c 200 http://localhost:8080/api/v1/market/depth/BTC-USDT?levels=20
```
...
| 事项 | 说明 |
|------|------|
| **必须用 `bench` profile** | 否则限流器（默认 500 QPS）会成为瓶颈 |
| **先充值再下单** | 内存账户无预置数据，不充值直接下单会报错 |
| **预挂对手盘** | 只挂买单再压测买单 = 0 成交，压不出撮合性能 |
| **关注非 2xx 比例** | wrk 输出的 `Non-2xx` 如果很多，说明触发了限流或业务异常 |
| **多核机器效果更好** | Disruptor 单线程撮合，但 HTTP 层是多线程的 |
