 ...
 * 撮合引擎条件化配置。
 * <p>
 * 通过 {@code apexmatch.engine.type} 配置项切换 Java、Rust、Golang 引擎：
 * <ul>
 *   <li>{@code java}（默认）：使用 Java 原生撮合引擎</li>
 *   <li>{@code rust}：通过 JNA 调用 Rust 动态库</li>
 *   <li>{@code golang}：通过 JNA 调用 Golang 动态库</li>
 * </ul>
 *
 * 配置示例：
 * <pre>
 ...
 *     golang-library-path: /opt/apexmatch/libapexmatch_go.dylib
 *     symbols:
 *       - BTC-USDT
 *       - ETH-USDT
 * </pre>
...
public class EngineConfig {
    ...
    /**
     * Golang 撮合引擎。
     * 当 apexmatch.engine.type=golang 时生效，且 classpath 中存在 GolangMatchingEngine 类。
     */
    @Bean
    @ConditionalOnProperty(name = "apexmatch.engine.type", havingValue = "golang")
    @ConditionalOnClass(name = "com.apexmatch.engine.golang.GolangMatchingEngine")
    public MatchingEngine golangMatchingEngine(
            @Value("${apexmatch.engine.golang-library-path}") String libraryPath) {
        try {
            Class<?> clazz = Class.forName("com.apexmatch.engine.golang.GolangMatchingEngine");
            MatchingEngine engine = (MatchingEngine) clazz
                    .getConstructor(String.class)
                    .newInstance(libraryPath);
            symbols.forEach(s -> engine.init(s, null));
            log.info("撮合引擎启动 [Golang 引擎], 动态库={}, 交易对={}", libraryPath, symbols);
            return engine;
        } catch (Exception e) {
            throw new IllegalStateException("无法加载 Golang 撮合引擎，请确保 apexmatch-engine-golang-adapter 在 classpath 中", e);
        }
    }
}
